...
from mundus.dataset.dataloader.dataset_loader_BCI_IV_c import DatasetLoader_BCI_IV_signle as Dataset
...
class Single_Noraml_Trainer(object):
    ...
    def __init__(self, args):
        ...
        self.trainset = Dataset('train', self.args, train_aug=False, subject_id=args.single_id)
        self.train_loader = DataLoader(dataset=self.trainset, batch_size=args.pre_batch_size, shuffle=True,
                                       num_workers=8, pin_memory=True)

        # Load meta-val set
        self.valset = Dataset('val', self.args, train_aug=False, subject_id=args.single_id)
        self.valset_loader = DataLoader(dataset=self.valset, batch_size=args.pre_batch_size, shuffle=True,
                                       num_workers=8, pin_memory=True)

        self.testset = Dataset('test', self.args, train_aug=False, subject_id=args.single_id)
...
class Single_Normal_Search_Trainer(object):
    """The class that contains the code for the pretrain phase."""
...
class Single_Searched_ReTrainer(object):
    """The class that contains the code for the pretrain phase."""
    ...
    def __init__(self, args):
       ...
       # Load pretrain set
        ...
        """
        test_model = self.model.net
        sample_input = torch.randn(1, 7, 400, 22)
        flops, params = profile(test_model, inputs=(sample_input, ), )
        ...
        summary(test_model, torch.zeros((1, 7, 400, 22)))
